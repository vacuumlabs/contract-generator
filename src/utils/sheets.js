...
export const loadSheetData = async (person, sheet) => {
  ...
  if (!header)
    throw new Error(
      `Spreadsheet https://docs.google.com/spreadsheets/d/${sheet.spreadsheetId} is empty. Check if sheet ID ${sheet.sheetId} and range ${sheet.range} are correct.`,
    )
  if (!header.includes(sheet.idSheetsColumn))
    throw new Error(
      `Field ${sheet.idSheetsColumn} not in header. Check the header of spreadsheet https://docs.google.com/spreadsheets/d/${sheet.spreadsheetId} - sheet ID ${sheet.sheetId} - range ${sheet.range}.`,
    )
